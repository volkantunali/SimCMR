...
class OslomRunner(object):
    ...
    def read_clusters(self, min_cluster_size):
        ...
        num_found = 0
        clusters = []
        # vt = self.last_result["output_dir"] + "\\" + OslomRunner.OUTPUT_FILE
        # print("bak: ", vt)
        with open(self.last_result["output_dir"] + "\\" + OslomRunner.OUTPUT_FILE, "r") as reader:
...
def run_in_memory(args, edges):
    ...
    # Finished
    logging.info("finished")
    return (clusters, oslom_log, result["time"])
